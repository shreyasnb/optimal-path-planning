...
def objective_function(polygon_flattened, *args):
    num_points = len(polygon_flattened) // 2
    polygon = np.reshape(polygon_flattened, (num_points, 2))
    points = args[0]
    
    # Ensure the polygon encloses all points
    if not all_inside_polygon(polygon, points):
        return np.inf  # Penalize infeasible solutions
    
    return -area_of_polygon(polygon)  # Negative because we minimize in scipy.optimize
...
def main():
    ...
    if len(polygon_points) < 10:
        ...
        num_points = len(scaled_polygon_points)
        
        # Flatten the points for optimization
        result = minimize(
            objective_function,
            scaled_polygon_points.flatten(),
            args=(points,),
            method='SLSQP',
            constraints={'type': 'eq', 'fun': lambda x: all_inside_polygon(np.reshape(x, (num_points, 2)), points)},
            options={'disp': True}
        )
        
        optimized_polygon_points = np.reshape(result.x, (num_points, 2))
        final_polygon_points = ensure_10_sided_polygon(optimized_polygon_points)

        print("Coordinates of the optimized 10-sided polygon:")
        print(final_polygon_points)

        plt.figure(figsize=(8, 6))
        plt.plot(points[:, 0], points[:, 1], 'o', label='Data Points')
        plt.fill(final_polygon_points[:, 0], final_polygon_points[:, 1], 'r-', alpha=0.3, label='Optimized 10-sided Polygon')
        ...
        plt.xlabel('X-axis')
        plt.ylabel('Y-axis')
        plt.title('Optimized 10-sided Polygon Enclosing All Points')
        plt.legend()
        plt.grid(True)
        plt.show()
