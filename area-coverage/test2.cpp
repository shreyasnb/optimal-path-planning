...
// Function to visualize the convex hull and points using matplotlib
void visualize_convex_hull(const vector<Point>& points, const vector<Point>& hull) {
    vector<double> x_points, y_points, x_hull, y_hull;
    ...
    for (const auto& point : hull) {
        x_hull.push_back(point.x);
        y_hull.push_back(point.y);
    }

    // Close the hull loop by adding the first point to the end
    x_hull.push_back(hull[0].x);
    y_hull.push_back(hull[0].y);

    // Plot the points
    plt::scatter(x_points, y_points, 10.0);
...
int main() {
    ...
    try {
        ...
        // Visualize the convex hull and points
        visualize_convex_hull(vertices, hull);
